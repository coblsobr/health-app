 ...
 * One captured page.
 *
 * Framing the ingredients and the directions separately was a dead end: it
 * made the user do the parser's job twice per recipe. The text comes back
 * fine — the splitting is what needs work, so the whole page is read and
 * `parseRecipeText` decides what is what.
 */
export type Shot = { uri: string; width: number; height: number };

/** One OCR line with where it sat on the page. Short keys: this gets shared. */
export type ScanLine = { t: string; x: number; y: number; w: number; h: number };
export type ScanPage = { w: number; h: number; lines: ScanLine[] };
...
/** Raw OCR with geometry. Position is the strongest signal for splitting a
 *  page into columns and sections, so it is captured even though the current
 *  parser works from plain lines. */
...
/** Flatten blocks to lines with rounded coordinates, in reading order. */
function toScanPage(blocks: OcrBlock[], shot: Shot): ScanPage {
  const lines: ScanLine[] = [];
  for (const block of blocks) {
    const src = block.lines?.length ? block.lines : [{ text: block.text, frame: block.frame }];
    for (const line of src) {
      if (!line.text?.trim()) continue;
      const f = line.frame;
      lines.push({
        t: line.text,
        x: Math.round(f?.left ?? 0),
        y: Math.round(f?.top ?? 0),
        w: Math.round(f?.width ?? 0),
        h: Math.round(f?.height ?? 0),
      });
    }
  }
  return { w: shot.width, h: shot.height, lines };
}

/**
 * Read one or more photos of a recipe and parse them as a single recipe.
 *
 * `pages` comes back alongside the result so a scan can be exported and
 * turned into a test case. Tuning the splitter against anything other than
 * what ML Kit actually returned would fix problems the phone does not have.
 */
export async function importFromPages(
  shots: Shot[]
): Promise<OcrRecipe & { rawText: string; pages: ScanPage[] }> {
  if (shots.length === 0) throw new Error('Take a photo first.');
  const pages: ScanPage[] = [];
  const texts: string[] = [];
  for (const shot of shots) {
    const { blocks, text } = await recognizeBlocks(shot.uri);
    pages.push(toScanPage(blocks, shot));
    texts.push(text);
  }
  const rawText = texts.join('\n');
